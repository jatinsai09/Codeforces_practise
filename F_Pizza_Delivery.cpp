...
int main() {
    ...
    while (tc--) {
        ...
        for (int i = 0; i < n; i++) {
            int j = i;
            while (j + 1 < n && v[i].first == v[j + 1].first) {
                j++;
            } 

            ll mn = v[i].second, mx = v[j].second;
            b.push_back({v[i].first, mx, mn});
            i = j;
        }
        ...
        for (int i = 1; i < b.size(); i++) {
            ...
            ndp[0] = min(dp[1] + abs(y1 - py1), dp[0] + abs(y1 - py2));
            ndp[0] += (x - px) + (y1 - y2);

            ndp[1] = min(dp[1] + abs(y2 - py1), dp[0] + abs(y2 - py2));
            ndp[1] += (x - px) + (y1 - y2);
