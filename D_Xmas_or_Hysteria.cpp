...
int main() {
    ...
    while (tc--) {
        ...
        auto print = [&] () -> void {
            cout << res.size() << nl;
            for (auto& it: res) {
                cout << it.first + 1 << sp << it.second + 1 << nl;
            }
        };
        ...
        if (m) {
            for (int i = n - 1; i >= m * 2; i--) {
                res.push_back({ind[i], ind[i - 1]});
            }
            for (int i = 0; i < m; i++) {
                res.push_back({ind[i], ind[i + m]});
            }
        } else {
            bool yes = false;
            ll req = a[ind[0]], got = 0;
            for (int i = 1; i < n; i++) {
                got += a[ind[i]];
                if (got >= req) {
                    yes = true;
                    for (int j = n - 1; j > i; j--) {
                        res.push_back({ind[j], ind[j - 1]});
                    }
                    for (int j = i; j >= 1; j--) {
                        res.push_back({ind[j], ind[0]});
                    }
                    break;
                }
            }
